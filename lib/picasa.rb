...
module Picasa
  ...
  class Picasa
    ...
    def tags(options = {})
      ...
      userId = options[:user_id] == nil ? self.picasa_session.user_id : options[:user_id]
      access = options[:access] == nil ? "public" : options[:access]
      url = "http://picasaweb.google.com/data/feed/api/user/#{userId}?kind=tag&access=#{access}"
      ...
      response, xml_response = http.get("#{uri.path}?#{uri.query}", headers)

      #xml_response = Net::HTTP.get_response(URI.parse(url)).body.to_s
      tags = create_tags_from_xml(xml_response)
    ...
    def create_tags_from_xml(xml_response)
      tags = []
      #response_hash = XmlSimple.xml_in(xml_response, { 'ForceArray' => false })
      #puts response_hash.inspect

      Picasa.entries(xml_response).each do |entry|
        #parse the entry xml element and get the tag object
        tag = Picasa.parse_tag_entry(entry)

        #enter session values in album object
        tag.picasa_session = PicasaSession.new
        tag.picasa_session.auth_key = self.picasa_session.auth_key
        tag.picasa_session.user_id = self.picasa_session.user_id
    ...
    def self.parse_tag_entry(tag_entry_xml)
      #album_hash = XmlSimple.xml_in(album_entry_xml.to_s, { 'ForceArray' => false })
      tag_hash = tag_entry_xml
      #p tag_hash
      tag = Tag.new
      tag.xml = tag_entry_xml.to_s
        
      tag.id = tag_hash["id"]
      tag.title = tag_hash["title"]["content"]
      
      # make self xml url
      links_from_hash = tag_hash["link"]
      if(links_from_hash.respond_to?(:each))
        links_from_hash.each do |link|
          if(link["rel"] == "self")
            tag.self_xml_url = link["href"]
          elsif(link["rel"] == "edit")
            tag.edit_url = link["href"]
          end
        end
      else
        tag.self_xml_url = nil
        tag.edit_url = nil
      end
  ...
  class Tag
    ...
    def photos
      ...
      userId = self.picasa_session.user_id
      tagName = self.title

      url = URI.escape("http://picasaweb.google.com/data/feed/api/user/#{userId}?kind=photo&tag=#{tagName}")
